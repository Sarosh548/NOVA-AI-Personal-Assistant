...
function errorText(error: unknown): string {
  return error instanceof ApiRequestError
    ? error.detail
    : "NOVA could not refresh the dashboard."
}
...
export function HomeSurface({ onNavigate }: HomeSurfaceProps) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const [tasks, reminders, activity, confirmations, workflows] =
      ...
      if (
        confirmations.status === "fulfilled" &&
        ...
        if (confirmations.status === "rejected") failures.push("approvals")
        if (workflows.status === "rejected") failures.push("workflows")
      }
      ...
      if (failures.length > 0) {
        setError(
          `Some dashboard data could not be refreshed: ${failures.join(", ")}.`,
        )
      }
    } catch (err) {
  ...
  return (
    <div className="content-shell home-dashboard">
      <section className="hero-grid dashboard-hero">
        <div>
          ...
          <p className="hero-copy">
            NOVA connects conversation, voice, planning, memory, knowledge, and
            authorized action in one personal workspace.
          </p>
        ...
        <div className="orb-card dashboard-orb" aria-label="NOVA live status">
          ...
          <div className="orb-status">
            <span className="status-pill">
              {loading || refreshing ? "Syncing" : "Ready"}
            </span>
            <span className="orb-caption">
              Your personal workspace is connected
            </span>
          </div>
        </div>
      </section>
      ...
      <section
        ...
        aria-label="Dashboard synchronization"
      >
      ...
      <section className="dashboard-stats">
        ...
        >
          ...
          <small>{activeTaskCount} in progress</small>
        </button>
        <button
          type="button"
          className="dashboard-stat-card"
        ...
        >
          ...
          <strong>{loading ? "—" : reminderCount}</strong>
          <small>
            {nextReminder ? formatDate(nextReminder) : "Nothing scheduled"}
          </small>
        </button>
        <button
          type="button"
        ...
        >
          ...
          <small>{latestActivity || "No new events"}</small>
        </button>
        <button
          type="button"
          className="dashboard-stat-card"
        ...
        >
          ...
          <small>Pending approvals + active workflows</small>
        </button>
      </section>
      ...
      <section className="section-block">
        ...
        <div className="action-grid">
          ...
          <ActionCard
            icon="check"
            title="Create a task"
            description="Capture work with priority and timing."
            onClick={() => onNavigate("Tasks")}
          />
          <ActionCard
            icon="bell"
            title="Set a reminder"
            description="Schedule a future follow-up."
            onClick={() => onNavigate("Reminders")}
          />
          <ActionCard
            icon="calendar"
            title="Plan the day"
            description="Connect Google Calendar and manage events."
            onClick={() => onNavigate("Calendar")}
          />
          <ActionCard
            icon="brain"
            title="Review memory"
            description="Inspect and correct NOVA's personal context."
            onClick={() => onNavigate("Memory")}
          />
          <ActionCard
            icon="book"
            title="Search knowledge"
            description="Manage the private retrieval layer."
        ...
        <div className="workspace-link-grid">
          <button type="button" onClick={() => onNavigate("Activity")}>
            ...
            <span>
              ...
              <small>See NOVA's operational trail.</small>
            </span>
          ...
          >
            ...
            <span>
              ...
              <small>
                Notifications, approvals, permissions, and workflows.
              </small>
            </span>
            <Icon name="arrow" size={16} />
          </button>
          <button type="button" onClick={() => onNavigate("Settings")}>
            ...
            <span>
              ...
              <small>Account and NOVA foundation details.</small>
            </span>
            <Icon name="arrow" size={16} />
          </button>
        </div>
